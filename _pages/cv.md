---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

Education
======
* **Université Paris Sciences et Lettres, Paris, France** _(Sep 2024 - Aug 2026)_  
  - (Joint program involving ENS, ENSCP, ESPCI, and Institut Curie)  
  - [_Master in Chemistry & Life Sciences_](https://www.chimie.ens.fr/formation/parcours-chimie-sdv/) (specialized in theoretical chemistry and modeling)  
  - First School Year Score: 15.15 / 20.00
* **Scuola Internazionale Superiore di Studi Avanzati (SISSA), Trieste, Italy** _((Upcoming) Jan 2026 - Aug 2026)_  
  - _Visiting Student_
* **Northwest University, Xi'an, China** _(Sep 2020 - Jun 2024)_  
  - _Bachelor of Science in Chemistry_  
  - GPA: 3.25 / 4.00

Research Experience
======
* **Intern, Theoretical and Scientific Data Science Group, SISSA, Trieste, Italy** _((Upcoming) Jan 2026 - Aug 2026)_  
  - _**The Dynamical Regime of Multi-area Brain Circuits**_ (Supervisor: [Francesca MASTROGIUSEPPE](https://fmastrogiuseppe.github.io/))  
  - Study the impact of introducing random inter-areal connections on the dynamical regimes with multi-area recurrent neural network models.
* **Intern, Gulliver Laboratory, ESPCI - PSL, Paris, France** _(Feb 2025 - Aug 2025)_  
  - _**Nonequilibrium Physics with Chemical Reaction Networks (CRNs)**_ (Supervisor: [David LACOSTE](https://dlacoste22.github.io/))  
  - Collaborated with [P. Gagrani](https://scholar.google.com/citations?hl=en&user=apGl8RAAAAAJ&view_op=list_works&sortby=pubdate) and [Y. De Decker](https://nlpc.ulb.be/dedecker.html); theoretically generalize topological parameters derived from network structures into intrinsic bounds on the dynamical growth rates of random CRNs with realistic kinetics by adapting von-Neumann growth model from economics, and verify results with numerical simulation.
* **Intern, Molecular Simulation Group, Northwest University, Xi'an, China** _(Oct 2023 - Jun 2024)_  
  - _**Scaling Laws for Protein Folding under Confinement**_ (Supervisor: [Tao CHEN](https://www.researchgate.net/scientific-contributions/Tao-Chen-82541369))  
  - Collaborated with [B. Zhu](https://www.x-mol.com/groups/chen_xiangyang/people/88865); conducted theoretical analysis and computational simulation with Langevin dynamics of coarse-grained protein models. Paper published (responsible for theoretical calculation and analysis of conformation entropy, bridging the relationship between scaling exponents and non-local contacts).

Publication
======
* **Statistical Simplicity of Random Chemical Reaction Networks** _(In preparation)_  
  - _**J. Wang**_, P. Gagrani, Y. De Decker and D. Lacoste
* **Scaling Laws for Protein Folding under Confinement** ([J. Phys. Chem. Lett. 2024, 15, 40, 10138–10145](https://doi.org/10.1021/acs.jpclett.4c02098))  
  - B. Zhu*, C. Zhang*, _**J. Wang**_, C. Jia, T. Lu, L. Dai and T. Chen (*Equal Contribution*)

Skills
======
* **Languages**  
  - **Mandarin Chinese**: Native Speaker  
  - **English**: Advanced (proficient in academic communication)  
  - _Japanese_: Basic (capable in listening, reading and basic communication)  
  - _French_: Beginner (familiar with basic daily expressions)
* **Programming**  
  - **Python**: Intermediate to Advanced (leveraged in a 6-month research internship)  
  - **C, C++, C#**: Intermediate (carried bachelor thesis work in C)  
  - _MATLAB/Simulink_: Experienced in data processing and analysis
* **Data & Image Analysis**  
  - _Origin_: Competent in data analyzing and visualization  
  - _ImageJ_: Competent in scientific image processing
* **Chemical Software**  
  - _LAMMPS_: Experienced in Molecular Dynamics simulation  
  - _PyMol_: Competent in macromolecular 3D structure visualization for Molecular Dynamics  
  - _Gaussian, GaussView_: Familiar with quantum chemistry calculations
